...
class PointPair {
  ...
  getHorizontalVerticalLinePointsPath(): Point[] {
    ...
    if (this.point1.x === this.point2.x) {
      const low: Point = this.point1.y < this.point2.y
        ? this.point1
        : this.point2;
      ...
      const low: Point = this.point1.x < this.point2.x
        ? this.point1
        : this.point2;
      const high: Point = low === this.point1 ? this.point2 : this.point1;
...
pointPairs.forEach((pp) => {
  pp.getHorizontalVerticalLinePointsPath().forEach((p) =>
    intersections[p.y][p.x]++
  );
});

const totalIntersections = intersections.reduce((count, intersectionRow) => {
  count += intersectionRow.reduce((rowCount, intersections) => {
    return intersections > 1 ? ++rowCount : rowCount;
  }, 0);
  return count;
}, 0);
