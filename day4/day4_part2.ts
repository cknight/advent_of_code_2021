...
(async function() {
    ...
    for (;bingoNumIndex < bingoNums.length; bingoNumIndex++) { //for each bingo number
        for(let j=0; j < boards.length; j++) { // for each board
            for (let x=0; x < boards[j].vals.length && !boards[j].isWinningBoard; x++) { // for each row
                for (let y=0; y < boards[j].vals[x].length && !boards[j].isWinningBoard; y++) { // for each column
